...
class Arbre:
    ...
    def prefixe(self):
        """ renvoie une liste correspondant au parcours prefixe
        de l'arbre """
        li = [self.racine]
        a = self
        rec_pref(a, li)
    ...
    def search(current_item, number):
        ...
        and any(search(item, number) for item in current_item)  # recursion
...
class ArbreBinaire(Arbre):
    ...
    def ajoute(self, a):
        ...
        if len(self._fils) < 2:
            ...
            raise RuntimeError('Pas plus de deux fils pour un arbre binaire')
